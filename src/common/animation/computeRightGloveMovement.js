...
export const computeForwardMovement = (
  ...
  if (punchType === "uppercut") {
    ...
    newRotation = {
      rightX: Math.min(
        xRotation - RIGHT_GLOVE_ROTATION.UPPERCUT_DELTA_X * FPSFactor,
        RIGHT_ANGLE,
      ),
      rightY: Math.max(
        yRotation - RIGHT_GLOVE_ROTATION.UPPERCUT_DELTA_Y * FPSFactor,
        -RIGHT_ANGLE,
      ),
      rightZ: Math.min(
        zRotation + RIGHT_GLOVE_ROTATION.UPPERCUT_DELTA_Z * FPSFactor,
        RIGHT_ANGLE - degToRad(40),
      ),
    };
  }
...
export const computeBackwardMovement = (
  ...
  if (punchType === "uppercut") {
    ...
    newPosition = {
      ...
      rightY: Math.min(
        yPosition + RIGHT_GLOVE_POSITION.UPPERCUT_DELTA_Y * FPSFactor,
        RIGHT_GLOVE_POSITION.INITIAL_Y,
      ),
      rightZ: Math.max(
...
export const computeTurningPoint = (
  ...
  if (
    punchType === "hook" &&
    gloveXPositionRef > MAX_GLOVE_REACH.HOOK_RIGHT_X
  ) {
  ...
  if (
    punchType === "uppercut" &&
    gloveYPositionRef <= RIGHT_GLOVE_POSITION.UPPERCUT_MIN_Y
  ) {
